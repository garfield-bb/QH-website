...
交付一个 **不写死业务数据**、**通过 API 实时拉取 HAP 数据** 的前端项目。
...
// ✅ 正确：真实、可用的示例数据
{
    ...
    image: [{
        name: 'sofa.jpg',
        url: 'https://m1.mingdaoyun.cn/doc/...'
    }],
    price: 3999,
    category: '客厅',
...
AskUserQuestion({
    questions: [{
        question: "新建的「新闻资讯」工作表需要示例数据以便前端页面预览效果。是否允许添加示例数据?",
        header: "示例数据",
        multiSelect: false,
        options: [
            {
                label: "添加示例数据(推荐)",
                description: "添加 5 条真实的新闻示例,包含标题、图片、内容等,便于查看页面效果"
            },
            {
                label: "添加最少数据",
                description: "仅添加 1-2 条简单示例,节省创建时间"
            },
            {
                label: "不添加数据",
                description: "工作表为空,前端页面将显示空状态提示"
            }
        ]
    }]
})
        ...
        rows: [
            {
                fields: [
                    ...
                    { id: '封面图字段ID', value: [{name: 'award.jpg', url: 'https://...'}] },
    ...
    const data = await API.getRows(CONFIG.WORKSHEETS.PRODUCTS, {
        filter: {
            ...
            children: [{
                ...
                operator: 'eq',
                value: [styleKey]  // 使用 key 而非 value
            }]
        }
    });
    return data.rows;
...
// 如果不知道 key，需要先获取选项列表
const getOptionKey = (options, displayValue) => {
